...
function scan() {
    ...
    docker run --rm --network "${CLI_NAME}" \
        ...
        -e SONAR_SCANNER_OPTS="-Dsonar.projectKey=${SONAR_PROJECT_NAME} -Dsonar.sources=${SONAR_SOURCE_PATH} -Dsonar.java.binaries=. -Dsonar.jacoco.reportPath=binaries/build/reports/jacoco.xml" \
